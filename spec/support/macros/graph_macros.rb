module GraphMacros
  ...
  def click_node(id)
    ...
    JS
    expect(page).to have_css('input.drawer-toggle:checked', visible: :all)
    expect(page).to have_css('turbo-frame#node-drawer')
    expect(page).to have_css("select#children_nodes")
  end
end
